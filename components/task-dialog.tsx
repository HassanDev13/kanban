...
export function TaskDialog({
  ...
  const [priority, setPriority] = React.useState(task?.priority || initialPriority);
  const [checklist, setChecklist] = React.useState<ChecklistItem[]>(task?.checklist || []);
  const [newChecklistItem, setNewChecklistItem] = React.useState("");
  const [isEditingDescription, setIsEditingDescription] = React.useState(mode === "add");
      ...
      <DialogContent className="max-w-4xl p-0 overflow-hidden h-fit" dir="rtl">
              ...
              <div>
                ...
                <div className="space-y-4">
                  <div className="flex gap-3">
                    ...
                    <div>
                      <p className="text-sm">
                        <span className="font-medium">المستخدم</span> أضاف هذه البطاقة إلى{" "}
                        <span className="font-medium">{column.title}</span>
                      </p>
                      <p className="text-xs text-muted-foreground">٢٤ فبراير، ٢٠٢٤</p>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div className="p-4 border-t flex justify-between">
          ...
          <div className="flex gap-2 mr-auto">
            ...
            <Button onClick={handleSave}>{mode === "add" ? "إضافة المهمة" : "حفظ التغييرات"}</Button>
          </div>
        </div>
